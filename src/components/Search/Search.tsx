...
export default function SearchMovies() {
  const { searchTerm, handleSearchChange, searchTrans } =
    useSearchState();
    ...
    <Fragment>
      <div className={style.container}>
        <h1 className={style.container__heading}>
          {searchTrans(
            TRANSLATIONCONSTANTSMOVIE.SEARCHMOVIEHEADING
          )}
        </h1>

        <form
          onSubmit={handleSearchSubmit}
          className={style["container__search-box"]}
        >
          <input
            className={style.container__input}
            type="text"
            placeholder={searchTrans(
              TRANSLATIONCONSTANTSMOVIE.PLACEHOLDERSEARCH
            )}
            value={searchTerm}
          ...
          >
            {searchTrans(
              TRANSLATIONCONSTANTSMOVIE.SEARCHBTN
            )}
          </button>
        </form>
      </div>
